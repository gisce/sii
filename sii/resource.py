...
def get_factura_recibida(invoice, rect_sust_opc1=False, rect_sust_opc2=False):
    ...
    if iva_values.get('inversion_sujeto_pasivo'):
        detalle_isp = iva_values.get('inversion_sujeto_pasivo')[0]
        desglose_factura.update({
            'InversionSujetoPasivo': {
                'DetalleIVA': [{
                    'TipoImpositivo': detalle_isp['TipoImpositivo'],
                    'BaseImponible': detalle_isp['BaseImponible'],
                    'CuotaSoportada': detalle_isp['CuotaSoportada']
                }]
            }
        })
